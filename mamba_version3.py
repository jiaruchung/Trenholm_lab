...
def ranksum_pval_matrix(cond1_samples,cond2_samples, bins=10):
    """
    Args:
...
#cond1
filespath=r'F:\Nathan\Ready_for_Python\x_obj1day1\con1'
...
plt.gca().set_title( 'Non-mouse object in upper left')
cond1_samples=collect_samples(aligned_data_cond1)
plt.savefig('heatplot_con1.png')

from scipy.ndimage.interpolation import zoom
from PIL import Image
from scipy import ndimage, misc
zoom_factor = 2.50 # 250% of the original image 
filename = "heatplot_con1.png"
img = Image.open(filename)
image_array = misc.fromimage(img)
zoomed_img = clipped_zoom(image_array, zoom_factor)
misc.imsave('heatplot_con1.png', zoomed_img)
#object_bouts
...
filespath=r'F:\Nathan\Ready_for_Python\x_obj1day1\con2' 
clean_pos_data(filespath, thres=1.1)
quad_analysis(filespath) 
occu_plots(filespath)
aligned_data_cond2=align_allMats(filespath, show_fig=True)
plt.gca().set_title( 'Mouse object in upper left')
cond2_samples=collect_samples(aligned_data_cond2)
plt.savefig('heatplot_con2.png')

#zoom in
zoom_factor = 2.50 # 250% of the original image 
filename = "heatplot_con2.png"
img = Image.open(filename)
image_array = misc.fromimage(img)
zoomed_img = clipped_zoom(image_array, zoom_factor)
misc.imsave('heatplot_con2.png', zoomed_img)


#cond1 vs cond2
result=ranksum_pval_matrix(cond1_samples,cond2_samples,10)
