# -*- coding: utf-8 -*-
"""
Created on Fri Nov 13 21:28:59 2020
...
def quad_analysis(filespath,framerate=30):
    ...
    for idx,file in enumerate(files): 
        ...
        xmin=132.718307495117; xmax=486.098205566406
        ymin=51.8144264221191; ymax= 418.037231445312
    
        #Visualize path plot, whole environment, occupied quadrant
        plt.figure(); plt.plot(pos_x,pos_y); plt.gca().invert_yaxis()
        
        plt.plot([xmin,xmin],[ymin,ymax],color='r') 
        plt.plot([xmax,xmax],[ymin,ymax],color='r') 
        
        plt.plot([xmin,xmax],[ymin,ymin],color='r') 
        plt.plot([xmin,xmax],[ymax,ymax],color='r') 
        ...
        for i in range(4):
            ...
            if i==0:
                quad=(x_cen>pos_x) & (y_cen>pos_y)  #UL
                #if obj_loc=='ul':
                #    plt.plot(pos_x[quad],pos_y[quad])
     
            elif i==1:
                quad=(x_cen< pos_x) & (y_cen>pos_y) #UR
                #if obj_loc=='ur':
                #    plt.plot(pos_x[quad],pos_y[quad])
    
            elif i==2:
                quad=(x_cen>pos_x) & (y_cen<pos_y) #BL
               #if obj_loc=='bl':
               #     plt.plot(pos_x[quad],pos_y[quad])
    
            else:
                quad=(x_cen<pos_x) & (y_cen<pos_y) #BR
                #if obj_loc=='br':
                #    plt.plot(pos_x[quad],pos_y[quad])
...
def occu_matrix(animal_pos,bins=10):
    ...
    Args: 
        ...
        bins: bin size for determining size of matrix. Default set to 12
    Returns: 
    ...
    occu_mat=np.flip(occu_mat,axis=0)
    return occu_mat
...
def align_allMats(filespath, show_fig=True):
    ...
    if show_fig:
        ...
        heatmap=plt.imshow(data, cmap='jet', interpolation='bilinear') # vmin=130, vmax=5700
        ax.set_yticks([])
        ax.set_xticks([])
...
#cond1
filespath=r'C:\Users\labuser\Desktop\Trenholm_lab-master\con1(non-mouse)\con1' 
clean_pos_data(filespath, thres=1.1)
...
#object_bouts
#obj_bouts_path=r'C:\Users\labuser\Desktop\Trenholm_lab-master\TEST_DATA\object_boutsFile' #it is is the only file with the required data
#object_bouts(obj_bouts_path,80)

#con2

filespath=r'C:\Users\labuser\Desktop\Trenholm_lab-master\con2(mouse)\con2' 
clean_pos_data(filespath, thres=1.1)
quad_analysis(filespath) 
occu_plots(filespath)
...
positive_values = substract[substract > 0]
negative_values = substract[substract < 0]

positive_values_df = pd.DataFrame(positive_values)
negative_values_df = pd.DataFrame(negative_values)

#p = sns.heatmap(positive_values_df, cmap='Reds', annot=False, cbar=False)
#n = sns.heatmap(negative_values_df, cmap='Blues', annot=False, cbar=False)
...
result_changePval=result_df[result_df<=0.05]

pval_increase= result_changePval[result_change>0]
pval_decrease= result_changePval[result_change<0]
#plt.style.use('classic')
plt.figure();plt.imshow(pval_increase)
sys.exit()
